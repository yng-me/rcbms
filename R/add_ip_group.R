...
add_ip_group <- function(.data, .ethnicity, .prefix) {
  ip_var <- paste0(tolower(.prefix), "_ethnic_group")
  .data <- .data |>
    dplyr::mutate(
      !!as.name(ip_var) := dplyr::case_when(
  ...
  attr(.data[[ip_var]], "type") <- "i"
  attr(.data[[ip_var]], "item") <- toupper(.prefix)
  attr(.data[[ip_var]], "label") <- "Ethnic group"
  attr(.data[[ip_var]], "valueset") <- data.frame(
    value = 1L:3L,
    label = c("IP groups", "Non-IP groups", "Foreign ethnicity")
  )
