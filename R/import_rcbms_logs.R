...
import_rcbms_logs <- function(.dir, .user_id, .dir_to = 'db-temp', .delete_source = T) {
  ...
  exdir_new <- file.path(.dir, 'db-temp-new')
  if(file.exists(exdir_new)) {
    unlink(exdir_new, recursive = T, force = T)
  }

  exdir <- create_new_folder(file.path(.dir, .dir_to))
  create_new_folder(exdir_new)
  ...
  for(i in seq_along(db_logs)) {
    ...
    db_log_i <- db_logs[i]
    db_name_i <- basename(db_log_i)
    input_data <- stringr::str_split_i(fs::path_ext_remove(db_name_i), '_', i = 1)
    ...
    if(grepl('^(hp|ilq|bp)$', input_data)) {
      ...
      if(grepl('_v3.db$', db_name_i)) {
        ...
        db_src_path <- file.path(exdir_new, db_name)

        migrate_rcbms_db(
          db_des = file.path(exdir_new, db_name),
          db_src = db_log_i,
          input_data = input_data
        )
      }

      res[[input_data]] <- sync_logs_db(
        db_src = db_src_path,
        db_des = file.path(db_dir, db_name),
        input_data = input_data
      )
