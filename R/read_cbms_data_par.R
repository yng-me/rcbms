...
read_cbms_data_par <- function(.references, .config) {
  ...
  for(i in seq_along(.config$input_data)) {
    ...
    if(is_bp_data & convert_to_parquet) {
      ...
      if(!exists('SUMMARY_RECORD__')) {
        SUMMARY_RECORD__ <- NULL
      }
      ...
      df_temp <- foreach::foreach(
        j = seq_along(df_files$unique$value),
        .final = function(x) {
          setNames(
            x,
            df_files$unique$value |>
              basename() |>
              tolower() |>
              stringr::str_remove_all(file_format)
          )
        }
      ) %dopar% {
        ...
        if(convert_to_parquet) {
          ...
          df_temp <- save_cbms_data(
            ...
            .summary_record = SUMMARY_RECORD__
          )

          if(is_first_record) {
            envir <- as.environment(1)
            assign("SUMMARY_RECORD__", df_temp, envir = envir)
          }
        }
        ...
        if(file.exists(pq_path) | file.exists(pq_dir)) {
          ...
          if(use_encryption) {
            ...
            ddd <- DBI::dbGetQuery(duckdb_conn, q_pq) |>
            ...
            if(partition) {
              ddd <- arrow::open_dataset(pq_dir)

            } else {
              ddd <- arrow::open_dataset(pq_path)
            }
          }
