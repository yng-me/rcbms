save_rcbms_logs <- function(
  ...
  save_current_logs(conn, .data, .input_data, log_tables, .config)
...
connect_to_rcbms_logs <- function(.config) {
  ...
  db_name <- paste0(wd, "/rcbms_logs.db")
...
save_current_logs <- function(.conn, .data, .input_data, .tables, .config) {
  ...
  hp_log_query <- DBI::dbSendQuery(.conn, "SELECT * FROM logs")
...
save_logs <- function(.config = getOption("rcbms.config")) {
  ...
  logs <- DBI::dbReadTable(conn, "logs") |>
    dplyr::tibble() |>
    dplyr::filter(id %in% current_logs_id)

  input_data <- unique(logs$input_data)
